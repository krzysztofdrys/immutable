...
func TestMap_Set(t *testing.T) {
	t.Run("Simple", func(t *testing.T) {
		...
		} else if k, v, ok := itr.Next(); k != 0 || v != 0 || ok {
			t.Fatalf("MapIterator.Next()=<%v,%v,%v>, expected false", k, v, ok)
		}
	})
	...
	t.Run("VerySmall", func(t *testing.T) {
		...
		itr := m.Iterator()
		for i := 0; i < n; i++ {
			if k, v, hasNext := itr.Next(); k != i || v != i+1 || !hasNext {
				t.Fatalf("MapIterator.Next()=<%v,%v,%v>, exp <%v,%v,%v>", k, v, hasNext, i, i+1, false)
			}
		}
		if !itr.Done() {
...
func (m *TMap) validateIterator(itr *MapIterator[int, int]) error {
	...
	if k, v, hasNext := itr.Next(); k != 0 || v != 0 || hasNext {
		return fmt.Errorf("map iterator returned key/value/hasNext after done: <%v/%v/%v>", k, v, hasNext)
	}
	return nil
}
...
func TestSortedMap_Set(t *testing.T) {
	...
	t.Run("ByteSliceKeys", func(t *testing.T) {
		m := NewSortedMap[[]byte, string](&byteSliceComparer{})
		m = m.Set([]byte("foo"), "bar")
		m = m.Set([]byte("baz"), "bat")
		m = m.Set([]byte(""), "EMPTY")
		if v, ok := m.Get([]byte("foo")); !ok || v != "bar" {
	...
	t.Run("Seek", func(t *testing.T) {
		...
		t.Run("Exact", func(t *testing.T) {
			...
			for i := 0; i < n; i += 2 {
				...
				for j := i; j < n; j += 2 {
					if k, _, _ := itr.Next(); k != fmt.Sprintf("%04d", j) {
		...
		t.Run("BeforeFirst", func(t *testing.T) {
			...
			itr.Seek("")
			for i := 0; i < n; i += 2 {
				if k, _, _ := itr.Next(); k != fmt.Sprintf("%04d", i) {
		...
		t.Run("AfterLast", func(t *testing.T) {
			itr := m.Iterator()
			itr.Seek("1000")
			if k, _, ok := itr.Next(); k != "" || ok {
				t.Fatalf("0. SortedMapIterator.Next()=%v,_,%v, expected 0,_,false", k, ok)
			} else if !itr.Done() {
				t.Fatalf("SortedMapIterator.Done()=true, expected false")
			}
		})
...
func (m *TSortedMap) validateForwardIterator(itr *SortedMapIterator[int, int]) error {
	for i, k0 := range m.keys {
		v0 := m.std[k0]
		if k1, v1, ok := itr.Next(); k0 != k1 || v0 != v1 || !ok {
	...
	if k, v, ok := itr.Next(); k != 0 || v != 0 || ok {
		return fmt.Errorf("SortedMapIterator.Next()=<%v,%v,%v>, expected false after done", k, v, ok)
	}
	return nil
}
...
func (m *TSortedMap) validateBackwardIterator(itr *SortedMapIterator[int, int]) error {
	...
	for i := len(m.keys) - 1; i >= 0; i-- {
		k0 := m.keys[i]
		v0 := m.std[k0]
		if k1, v1, ok := itr.Prev(); k0 != k1 || v0 != v1 || !ok {
			return fmt.Errorf("%d. SortedMapIterator.Prev()=<%v,%v,%v>, expected <%v,%v,%v>", i, k1, ok, v1, k0, v0, false)
		}
	...
	if k, v, ok := itr.Prev(); k != 0 || v != 0 || ok {
		return fmt.Errorf("SortedMapIterator.Prev()=<%v,%v,%v>, expected false after done", k, v, ok)
	}
	return nil
}
